...
class AdvancedPatternAnalyzer(BaseAgent):
    """
    סוכן מתקדם לניתוח תבניות מורכבות וניתוח שוק
    מתמחה בתבניות מבוססות למידה וניתוח יחסי
    """
    ...
    def __init__(self, config=None):
        ...
        self.log("Advanced Pattern Analyzer initialized")

    def analyze(self, symbol: str, price_df: pd.DataFrame = None, market_data: pd.DataFrame = None) -> Dict:
        """
        ...
        try:
            ...
            # ניתוח יחסי לשוק
            market_analysis = self._analyze_market_relative(symbol, price_df, market_data)
            ...
            # חישוב ציון כולל
            score = self._calculate_advanced_score(complex_patterns, market_analysis, volume_analysis, trend_analysis, support_resistance)
            
            # יצירת סיכום מתקדם
            summary = self._generate_advanced_summary(complex_patterns, market_analysis, volume_analysis, trend_analysis, support_resistance)
            
            # המלצות מתקדמות
            recommendations = self._generate_advanced_recommendations(complex_patterns, market_analysis, volume_analysis, trend_analysis, support_resistance)
    ...
    def _detect_advanced_cup_handle(self, price_df: pd.DataFrame) -> Dict:
        ...
        high_prices = price_df['high'].tail(60)
        low_prices = price_df['low'].tail(60)
        ...
        cup_volume = volume[:cup_length]
        handle_volume = volume[cup_length:]
        
        # בדיקת תנאי Cup מתקדמים
        ...
        # נפח מתקדם
        cup_volume_pattern = self._analyze_cup_volume_pattern(cup_volume)
        handle_volume_pattern = self._analyze_handle_volume_pattern(handle_volume)
        ...
        return {
            ...
            "cup_volume_pattern": cup_volume_pattern,
            "handle_volume_pattern": handle_volume_pattern
        }
    ...
    def _check_rounded_bottom(self, prices: pd.Series) -> bool:
        ...
        return avg_deviation < 0.02  # סטייה של פחות מ-2%
    ...
    def _detect_falling_wedge(self, price_df: pd.DataFrame) -> Dict:
        ...
        # Falling Wedge: קו עליון יורד מהר יותר מקו תחתון
    ...
    def _detect_channel_breakout(self, price_df: pd.DataFrame) -> Dict:
        ...
        breakout = current_price > current_channel_high * 1.02  # פריצה של 2%
    ...
    def _detect_consolidation_breakout(self, price_df: pd.DataFrame) -> Dict:
        ...
        is_consolidation = price_range < 0.1  # טווח של פחות מ-10%
    ...
    def _analyze_market_relative(self, symbol: str, price_df: pd.DataFrame, market_data: pd.DataFrame = None) -> Dict:
        """ניתוח יחסי לשוק"""
        try:
            ...
            # ביצועים ב-20 ימים
            performance_20d = (close_prices.iloc[-1] / close_prices.iloc[-20] - 1) * 100
            
            # ביצועים ב-50 ימים
            performance_50d = (close_prices.iloc[-1] / close_prices.iloc[-50] - 1) * 100 if len(close_prices) >= 50 else 0
            ...
            overall_score = (performance_score + volatility_score + trend_score) / 3
            ...
            self.log(f"שגיאה בניתוח יחסי לשוק: {str(e)}")
            return {
                "performance_20d": 0,
                "performance_50d": 0,
                "volatility": 0,
    ...
    def _detect_volume_divergence(self, prices: pd.Series, volume: pd.Series) -> bool:
        ...
        # בדיקה אם המחיר עולה אבל הנפח יורד
    ...
    def _detect_accumulation(self, prices: pd.Series, volume: pd.Series) -> bool:
        ...
        # הצטברות: נפח גבוה במחירים נמוכים
    ...
    def _calculate_trend(self, prices: pd.Series) -> float:
        ...
        return slope / prices.mean()  # נרמול
    ...
    def _calculate_trend_consistency(self, trends: Dict) -> float:
        ...
        # בדיקה אם כל המגמות באותו כיוון
    ...
    def _calculate_pattern_quality(self, patterns: Dict) -> float:
        ...
        # חישוב איכות על בסיס חוזק ועקביות
        strengths = [p.get('strength', 0) for p in detected_patterns]
        avg_strength = np.mean(strengths)
        
        # בונוס על מספר תבניות
        pattern_bonus = min(20, len(detected_patterns) * 5)
        
        return min(100, avg_strength + pattern_bonus)

    def _calculate_advanced_score(self, complex_patterns: Dict, market_analysis: Dict, volume_analysis: Dict, trend_analysis: Dict, support_resistance: Dict) -> int:
        """חישוב ציון מתקדם"""
        # משקלים
        pattern_weight = 0.35
        market_weight = 0.25
    ...
    def _generate_advanced_summary(self, complex_patterns: Dict, market_analysis: Dict, volume_analysis: Dict, trend_analysis: Dict, support_resistance: Dict) -> str:
        """יצירת סיכום מתקדם"""
        summary_parts = []
    ...
    def _generate_advanced_recommendations(self, complex_patterns: Dict, market_analysis: Dict, volume_analysis: Dict, trend_analysis: Dict, support_resistance: Dict) -> List[str]:
