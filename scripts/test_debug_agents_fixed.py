...
def test_problematic_agents():
    ...
    try:
        ...
        # רשימת הסוכנים הבעייתיים עם השמות הנכונים
        problematic_agents = [
            ("bullish_pattern_spotter", "BullishPatternSpotter"),
            ("adx_score_agent", "ADXScoreAgent"), 
            ("trend_shift_detector", "TrendShiftDetector"),
            ("parabolic_agent", "ParabolicAgent"),
            ("moving_average_pressure_bot", "MovingAveragePressureBot")
        ]
        
        results = {}
        
        for module_name, class_name in problematic_agents:
            try:
                print(f"\n📊 בדיקת {class_name}...")
                ...
                try:
                    ...
                    if len(sig.parameters) == 2:
                        ...
                        # התאמת נתונים לכל סוכן
                        if class_name == "ADXScoreAgent":
                            # ADXScoreAgent צריך high, low, close
                            adapted_data = sample_data.copy()
                            adapted_data.columns = [col.lower() for col in adapted_data.columns]
                            result = agent.analyze('TEST', adapted_data)
                        elif class_name == "TrendShiftDetector":
                            # TrendShiftDetector צריך symbol ב-__init__
                            try:
                                # ניסיון עם symbol ב-__init__
                                agent = agent_class('TEST')
                                result = agent.analyze('TEST', sample_data)
                            except:
                                # ניסיון ללא symbol
                                agent = agent_class()
                                result = agent.analyze('TEST', sample_data)
                        elif class_name == "ParabolicAgent":
